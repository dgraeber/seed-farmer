...
with Diagram("SeedFarmer Cross Account Deployments with Module Roles", show=False, direction="TB"):
    ...
    with Cluster("Deployment Accounts"):
        ...
        with Cluster("Account C"):
            ...
            module_role_c = IAMRole("Module Role")
            s3_c = S3("S3 Bucket")
    ...
    codebuild_b >> Edge(label="4. Assumes", color="darkblue") >> module_role_b
    codebuild_c >> Edge(label="5. Assumes", color="darkblue") >> module_role_c
    ...
    module_role_b >> Edge(label="Deploys", color="darkblue", style="bold") >> s3_b
    module_role_b >> Edge(label="Deploys", color="darkblue", style="bold") >> secrets_b
    ...
    # Account C - Module Role access (for actual deployments)
    module_role_c >> Edge(label="Deploys", color="darkblue", style="bold") >> s3_c
    module_role_c >> Edge(label="Deploys", color="darkblue", style="bold") >> secrets_c
